...
Details can be found in the [LOAD FROM documentation page](//kuzudb.github.io/docs/cypher/load_from).
...
If a header is manually specified, Kuzu will try to cast to the given type and throw exceptions if casting fails. More information can be found [here](//kuzudb.github.io/docs/cypher/load_from).
...
The above sequence of statements starts a write transaction, adds a new node, and within the same transaction also reads all of the tuples in User table before committing the transaction. More info on the new transaction statement can be found [here](//kuzudb.github.io/docs/cypher/transaction).
...
For more information, check out [the new documentation](//kuzudb.github.io/docs/cypher/query-clauses/match#project-intermediate-nodes-and-rels).
...
See [Create Table](//kuzudb.github.io/docs/cypher/data-definition/create-table) for more information.
...
We have also added several casting and list functions. See [functions](//kuzudb.github.io/docs/cypher/expressions/) for more information.
...
If multi-line strings are present, the CSV reading will fail, and you will need to fall back to single thread mode by setting `parallel=false`. See [Data Import from CSV Files](//kuzudb.github.io/docs/data-import/csv-import).
