...
Internally, a path is processed as a `STRUCT` with two fields, a nodes field with key `_NODES` and type `LIST[NODE]` and a rels field with key `_RELS` and type `LIST[REL]`. See [`PATH`](//kuzudb.github.io/docs/cypher/data-types/path) for details. Users can access nodes and rels field with `nodes(p)` and `rels(p)` function calls as follows:
...
See [All Shortest Paths](//kuzudb.github.io/docs/cypher/query-clauses/match#all-shortest-path) on our documentation for more information.
...
More built in procedures can be found [here](//kuzudb.github.io/docs/cypher/query-clauses/call).
...
More configuration options can be found [here](//kuzudb.github.io/docs/cypher/configuration).
...
More information on the blob data type can be found [here](//kuzudb.github.io/docs/cypher/data-types/blob).

## Client APIs: Rust and Java
In this release, we're expanding the accessibility of Kuzu, bridging the gap with some of the most popular programming languages in the developer community. Specifically, we now have [Rust](//kuzudb.github.io/docs/client-apis/rust) and [Java](//kuzudb.github.io/docs/client-apis/java) APIs.
...
For a more detailed overview of our testing framework, please visit [here](//kuzudb.github.io/docs/development/testing-framework).
