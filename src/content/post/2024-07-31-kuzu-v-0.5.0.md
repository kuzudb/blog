...
Kuzu's [httpfs](https://kuzudb.github.io/docs/extensions/httpfs) extension allows scanning and copying from a remote file system (e.g., S3 or HTTPS).
...
[^1]: Cached files are visible per transaction. If you run a `LOAD FROM` statement on a remote file, then this file will be downloaded first and then scanned locally from the downloaded file. If you run the same `LOAD FROM` statement again, it will be downloaded again from the remote URL. This is because the second statement is executed as a separate transaction and we do not know if the already downloaded remote file has changed since the last time Kuzu downloaded it. If you need to scan the same remote file multiple times and benefit from caching across multiple scans, you can run all the `LOAD FROM` statements in the same transaction. See the [docs](https://kuzudb.github.io/docs/extensions/httpfs#local-cache-for-remote-files) for more details.
...
You can use the [remote file system cache](https://kuzudb.github.io/docs/extensions/httpfs#local-cache-for-remote-files) feature
...
See this [documentation page](https://kuzudb.github.io/docs/extensions/remote-kuzu)
...
See this [documentation page](https://kuzudb.github.io/docs/client-apis/python#udf) for more information on Python UDFs.
...
See the [documentation page](https://kuzudb.github.io/docs/import/copy-from-dataframe) for more
...
Check out more details in this [documentation page](https://kuzudb.github.io/docs/import/copy-from-dataframe).
...
```
Checkout more details in this [documentation page](https://kuzudb.github.io/docs/extensions/rdbms).
...
Checkout more details in this [documentation page](https://kuzudb.github.io/docs/extensions/json). In a future release, we also plan to release a feature
that provides a JSON data type in the system, allowing users to store JSON documents as node or relationship properties.

## Decimal data type

For users who require exact precision in their floating-point numbers, we've introduced the [`DECIMAL` data type](https://kuzudb.github.io/docs/cypher/data-types#decimal),
...
```
See this [documentation page](https://kuzudb.github.io/docs/cypher/data-definition/create-table#if-not-exists) for further details.
...
See this [documentation page](https://kuzudb.github.io/docs/cypher/data-definition/drop#if-exists) for further details.
...
![](/blog/img/2024-07-31-release-0-5-0/progress_bar.gif)
...
See the [details here](https://kuzudb.github.io/docs/developer-guide/join-order-hint).
